    ...
    async _initiateExpressPayment(ev) {
        ...
        if (shippingInformationRequired){
            const shippingInfo = document.querySelector(
                `#o_payment_negdi_shipping_info_${providerId}`
            );
            expressDeliveryAddress = {
                'name': shippingInfo.querySelector('#o_payment_negdi_shipping_name').value,
                'email': shippingInfo.querySelector('#o_payment_negdi_shipping_email').value,
                'street': shippingInfo.querySelector('#o_payment_negdi_shipping_address').value,
                'street2': shippingInfo.querySelector('#o_payment_negdi_shipping_address2').value,
                'zip': shippingInfo.querySelector('#o_payment_negdi_shipping_zip').value,
                'city': shippingInfo.querySelector('#o_payment_negdi_shipping_city').value,
                'country': shippingInfo.querySelector('#o_payment_negdi_shipping_country').value,
            };
